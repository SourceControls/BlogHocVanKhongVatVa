...
import {createPost, useTags, publishPost, updatePost, requestPublish, hidePost} from '@util'
...
export default function MyEditor({post, close, mutate, posts}) {
    ...
    const handleMutate = async (updatedPost) => {
        ...
        const newItems = posts.map((item) => (item.postItem == updatedPost.postItem ? updatedPost : item))
    ...
    const handleSubmit = (status) => {
        if (editorRef.current) {
            if (!post) {
                //create new
                createPost(form.values).then((rs) => {
                ...
                if (status == 'hide') {
                    ...
                    updatePost(form.values).then((rs) => {
                        handleMutate(rs)
                    })
